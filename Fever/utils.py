...
def read_full(args, module, slice = True,  full_num = 3000, sampled = True):
    if not sampled:
        orig_sampled = args.use_sampled
        args.use_sampled = False
    new_args = copy.deepcopy(args)
    new_args.dev_slice = 0
    new_args.num_dev = full_num
    predictions = read_path(module.result_cache_name(new_args))
    print('reading from ', module.result_cache_name(new_args))
    if slice:
        predictions = predictions[args.dev_slice:args.num_dev]
    if not sampled:
        args.use_sampled = orig_sampled
    return predictions
