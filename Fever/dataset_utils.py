...
def read_fever_data(fname, manual_annotation_style=None):
    dataset = read_jsonl(fname)
    if 'paper_test' in fname:
        shuffled_indices = np.loadtxt('data/shuffled_indices.out', dtype=int)
        data = [dataset[i] for i in shuffled_indices]
        print('shuffled')
    else:
        data = dataset
    ...
    if manual_annotation_style is not None:
        ...
        data = [x for x in data if str(x["id"]) in man_rat_dict.keys()] # select the data with manual rationales
    else:
        man_rat_dict = dict()
    ...
    for d in data:
        ex = {
            "id": d["id"],
            "question": d["claim"].lstrip(),
            "label": d["label"],
            'evidence': d['evidence']
        }
        if str(d["id"]) in man_rat_dict.keys():            
            ex["manual_rationale"] = man_rat_dict[str(d["id"])] #record the manual rationale if exists
        examples.append(ex)   
    return examples
