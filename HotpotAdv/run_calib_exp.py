...
def load_train_test_split(args):
    ...
    TEST_PART = 250
    # train_pairs = pairs[:-TEST_PART] #USE FIRST 250 AS TRAIN
    # test_pairs = pairs[-TEST_PART:]
    train_pairs = pairs[:(args.sub_calib - 6)]
    test_pairs = pairs[(args.sub_calib - 6):]
...
class Calibrator:
    ...
    def test(self, examples, predictions):
        ...
        print("C: {:.2f}".format(f1auc_score(
                calib_scores, acc_scores))) #USE THIS ONE
        print('accuracy: ', np.mean(acc_scores))
        print('f_1: ', np.mean(f1_scores))
