...
def _parse_args():
    ...
    specify_engine(args)
    args.helper = get_joint_prompt_helper('e-p')
    return args
...
def get_evaluation_results(examples, predictions):
    ...
    for ex, pred in zip(examples, predictions):
        gt_ans = ex['answer']
        p_ans = pred['answer']
        acc, (f1, pre, rec), _ = wiki_evaluation(p_ans, ex["answer"])
...
def load_train_test_split(args):
    ...
    examples = read_wikiqa_data(f"data/dev_sampled.json")
...
def process_feature_and_label(examples, predictions):
    ...
    for ex, pred in zip(examples, predictions):        
        rat_val = rationale_validness_quality(pred['rationale'], ex['all_pars'], ex['question'], ex['answer'])
